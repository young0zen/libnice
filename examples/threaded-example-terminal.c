...
int m_fd; //为了让recvcallback也可以忘master里写东西
...
//  }
 ...
 // }
  // start the debug mode
   if((master_fd = /*posix_openpt*/ open("/dev/ptmx",O_RDWR|O_NOCTTY)) == -1){
	  g_error("wrong in create master ptmx");
	  exit(-1);
  }
  m_fd = master_fd;
  if(grantpt(master_fd) == -1){
	  g_error("wrong in grant master ptmx");
   	  exit(-1);
  }
  if(unlockpt(master_fd) == -1){
	  g_error("wrong in unlock master ptmx");
   	  exit(-1);
  }
  if((slave_path=ptsname(master_fd)) == NULL){
	  g_error("wrong in getting pts slave path");
   	  exit(-1);
  }
#ifdef G_OS_WIN32
  mchannel = g_io_channel_win32_new_fd(master_fd);
#else
  mchannel = g_io_channel_unix_new(master_fd);
#endif
  g_io_channel_set_flags (mchannel, G_IO_FLAG_NONBLOCK, NULL); 
  pid = fork();
  if(pid == 0)
  {
	//获取slave的文件描述符
	if((slave_fd = open(slave_path,O_RDWR)) == -1){
          g_error("wrong in open slave_fd");
          exit(-1);
 	 }
	//检查是否具有三个流模块
//	if((set_up = ioctl(slave_fd,I_FIND,"ldterm")) < 0){
//	  g_error("wrong in get set_up");
//          exit(-1);
//	}
//	//若没有就添加
//	if(set_up == 0){
...
//	}
	//新建session,脱离控制终端
	if(setsid() < 0)
		g_error("setsid error");
	if(ioctl(slave_fd,TIOCSCTTY,0) < 0)
		g_error("TIOCSCTTY ERROR");
	//将slave输入输出重定向
	if(dup2(slave_fd,STDIN_FILENO) != STDIN_FILENO)
		g_error("dup2 error to stdin");
