...
//  }
   // flags = g_io_channel_get_flags(io_stdin);
   //g_io_channel_set_flags(io_stdin, flags|G_IO_FLAG_NONBLOCK,NULL);
//    printf("begin debug  (Ctrl-D to quit):\n");
//    fflush (stdout);
  system("stty -echo");
  system("stty raw");  
  while (!exit_thread) {
	    buf_command[0] =  getchar();
            
	    if(buf_command[0]  == EOF)
	   	{
		nice_agent_send(agent, stream_id, 1, 1, "\0");
		break;
		}
	    else
		nice_agent_send(agent, stream_id, 1, 1, buf_command);

	    
//	   if( read(0,buf_command,1) > 0)
//	   {
//		nice_agent_send(agent, stream_id, 1, 1, buf_command);
//	   }
//	   else {
...
//		
//	   }
//     /*GIOStatus*/ s = g_io_channel_read_chars (io_stdin, buf_command, 1, NULL, NULL);
//     if (s == G_IO_STATUS_NORMAL) {
//       nice_agent_send(agent, stream_id, 1, 1, buf_command);
//     } else if (s == G_IO_STATUS_AGAIN) {
//       g_usleep (100000);
